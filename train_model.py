...
Trains SEVERAL classification algorithms on the historical accident data,
compares them on held-out test data, and saves:
  - model_artifacts/best_model.pkl        (the winning trained model)
  - model_artifacts/scaler.pkl            (feature scaler, needed by LR/SVM)
  - model_artifacts/model_comparison.csv  (leaderboard of all algorithms)
  - model_artifacts/feature_importance.csv (if the winner supports it)

Run:
    python data/generate_historical_data.py   # only needed once / to refresh
...
def main():
    ...
    NEEDS_SCALING = {"Logistic Regression"}  # tree ensembles don't need scaling
